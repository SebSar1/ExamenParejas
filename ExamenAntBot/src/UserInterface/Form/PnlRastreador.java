...
public class PnlRastreador extends JPanel {
    ...
    private void showHormigasTabla() {
        try {
            String[] encabezado = { "IdHormigaTipo", "IdHormigaTipoPadre", "Nombre", "Estado", "FechaCrea" };

            // Obtener los datos de la base de datos
            ArrayList<HormigaTipoDTO> Hormigas = HormigaTipoBL.getAll();
            ...
            for (int i = 0; i < Hormigas.size(); i++) {
                HormigaTipoDTO Hormiga = Hormigas.get(i);
                data[i][0] = Hormiga.getIdHormigaTipo();
                data[i][1] = Hormiga.getIdHormigaTipoPadre();
